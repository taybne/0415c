...
>>>>>>> f7b72ac90dfe668b0bdbed3e54cea5cbb549e539
# ===== PATH =====
...
GUEST_TELEGRAM_ID = 'tag-guest-auto'
...
def get_or_create_guest_user():
    user = User.query.filter_by(telegram_id=GUEST_TELEGRAM_ID).first()
    if user:
        return user
...
class Friend(db.Model):
    ...
    friend_id = db.Column(db.String(50), db.ForeignKey('user.id'))

class Message(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    sender = db.Column(db.String(50), db.ForeignKey('user.id'))
    receiver = db.Column(db.String(50), db.ForeignKey('user.id'))
...
# чат и пользователи
admin.add_view(sqla.ModelView(User, db.session))
admin.add_view(sqla.ModelView(Friend, db.session))
admin.add_view(sqla.ModelView(Message, db.session))

# ===== API =====

# helper to make sure user exists
...
def get_or_create_user(user_id, nickname=None):
    ...
    if not user:
        user = User(id=user_id, nickname=nickname or user_id)
        db.session.add(user)
        db.session.commit()
    return user
...
def search_users():
    ...
    if not q:
        return jsonify([])
    matches = User.query.filter(User.nickname.ilike(f"%{q}%")).all()
    return jsonify([{'id': u.id, 'nickname': u.nickname} for u in matches])
...
def friend_request():
    ...
    get_or_create_user(receiver)
    if not Friend.query.filter_by(user_id=sender, friend_id=receiver).first():
        db.session.add(Friend(user_id=sender, friend_id=receiver))
    if not Friend.query.filter_by(user_id=receiver, friend_id=sender).first():
        db.session.add(Friend(user_id=receiver, friend_id=sender))
>>>>>>> f7b72ac90dfe668b0bdbed3e54cea5cbb549e539
    db.session.commit()
...
def get_friends(user_id):
    relations = Friend.query.filter_by(user_id=user_id).all()
    out = []
    for rel in relations:
        u = User.query.get(rel.friend_id)
        if u:
            out.append({'id': u.id, 'nickname': u.nickname})
    return jsonify(out)

# 🔹 СООБЩЕНИЯ МЕЖДУ ДВУМЯ ПОЛЬЗОВАТЕЛЯМИ
@app.route('/api/messages/<u1>/<u2>')
def get_messages(u1, u2):
    ...
    return jsonify([
        {'sender': m.sender, 'receiver': m.receiver, 'text': m.text, 'created_at': m.created_at.isoformat()}
        for m in msgs
    ])
...
def send_message():
    ...
    text = data.get('text')
    if not s or not r or not text:
        return jsonify({'error': 'sender_id, receiver_id and text required'}), 400
    get_or_create_user(s)
    get_or_create_user(r)
    msg = Message(sender=s, receiver=r, text=text)
    db.session.add(msg)
    db.session.commit()
    return jsonify({'status': 'ok'})
...
=======
    ...
    if data.get('user_id'):
        get_or_create_user(data.get('user_id'), data.get('nickname'))
>>>>>>> f7b72ac90dfe668b0bdbed3e54cea5cbb549e539
...
if __name__ == "__main__":
    ...
    with app.app_context():
        ...
        # Add Beijing if not exists
        beijing = City.query.filter_by(slug='beijing').first()
        if not beijing:
            beijing = City(name='Пекин', slug='beijing')
        ...
        locations_data = [
            ...
            {
                'title': 'Хуамин парк',
                'description': 'Парк Хуамин в Москве с зелёными зонами и прогулочными маршрутами.',
                'theme': 'leisure',
                'photos': json.dumps([
                    '/static/photos/moscow/хуамин1.png',
                    '/static/photos/moscow/хуамин2.png'
                ]),
                'city_id': moscow.id
            },
            {
                'title': 'Lovin',
                'description': 'Кривоколенный пер., 3, Москва — смэш-бургерная у Лубянки с дайнер-настроением, бургерами, фри и милкшейками.',
                'theme': 'restaurant',
                'photos': json.dumps([
                    '/static/photos/moscow/lovin.png',
                    '/static/photos/moscow/lovin2.png',
                    '/static/photos/moscow/lovin3.png',
                    '/static/photos/moscow/lovin4.png'
                ]),
                'city_id': moscow.id
            },
            {
